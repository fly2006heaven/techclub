...
```
techclub/
├── index.html         # 首页：结构 + 内联 CSS + 内联 JS
├── profile.html       # 个人中心（占位彩蛋页）：全屏背景图 + 中央文案 + 返回按钮
├── assets/
│   └── profile-bg.png # profile.html 的全屏背景图（1851×1433）
└── README.md          # 本文件
```
...
```text
index.html ──登录/注册（弹窗提交成功）──▶ 导航栏显示头像下拉菜单
     │                                          │
     │                                          └─「个人中心」──▶ profile.html
     │                                                                  │
     └────────────────────「返回」按钮 ／ 历史回退 ◀────────────────────┘
...
`profile.html`（个人中心）是一个**占位彩蛋页**：全屏背景图 + 居中大字「你不会以为真的有吧？」+ 右上角返回按钮。
返回逻辑为 `history.back()` 回退上一页；若本页是被直接输入网址打开的（无可回退历史），自动降级为跳转 `index.html`，避免"点了没反应"。
